 ...
 */
class Day14(val tinyInput: List<String>) {

    fun part1(): Long {
        val memoryState = MemoryState(mutableMapOf(), listOf())
        parseInput().forEach { input ->
            when (input) {
                is Mask -> memoryState.mask = input.mask
                is MemorySave -> {
                    memoryState.memory[input.address] = applyMask(memoryState.mask, input.toSave)
                }
                else -> throw RuntimeException()
            }
        }
        return memoryState.memory.entries.map { it.value }.sum()
    }
    ...
    fun part2(): Any? {
        ...
        parseInput().forEach { input ->
            when (input) {
                is Mask -> memoryState.mask = input.mask
                is MemorySave -> {
                    applyMaskV2(memoryState.mask, input.address).forEach {
//                        println("saving ${input.toSave} to $it")
                        memoryState.memory[it] = input.toSave
                    }
                }
                else -> throw RuntimeException()
            }
    ...
    data class MemoryState(val memory: MutableMap<Long, Long>, var mask: List<Char>)
    interface InputLine
    data class Mask(val mask: List<Char>) : InputLine
    data class MemorySave(val address: Long, val toSave: Long) : InputLine

    fun applyMask(mask: List<Char>, int: Long): Long {
        return mask.zip(Integer.toBinaryString(int.toInt()).padStart(mask.size, '0').toCharArray().toList())
            .map { (mask, maskee) -> if (mask == 'X') maskee else mask }.joinToString("")
            .toLong(2)
    }

    fun applyMaskV2(mask: List<Char>, maskee: Long): List<Long> {
        val toMask = Integer.toBinaryString(maskee.toInt()).padStart(mask.size, '0').toCharArray().toList()
//        val first = translateMaskChar(mask.first(), toMask.first())
        return mask.foldIndexed(
            listOf(listOf<Char>()),
            { i, soFar, nextMaskChar ->
                val translateMaskChar = translateMaskChar(nextMaskChar, toMask[i])
//                println("$soFar, $nextMaskChar")
                soFar.flatMap { s -> translateMaskChar.map { n -> s + n } }
            }
        ).map { it.joinToString("").toLong(2) }

    }

    private fun translateMaskChar(mask: Char, address: Char): List<Char> {
        return when (mask) {
            '0' -> listOf(address)
            '1' -> listOf('1')
            else -> listOf('0', '1')
    ...
    fun parseInput(): List<InputLine> {
        return tinyInput.map {
            when {
                it.startsWith("mask") -> Mask(it.removePrefix("mask = ").toList())
                else -> {
                    val (addr, toSave) = Regex("mem\\[(\\d+)\\] = (\\d+)").matchEntire(it)!!.destructured
                    MemorySave(addr.toLong(), toSave.toLong())
                }
            }
        }
    }
